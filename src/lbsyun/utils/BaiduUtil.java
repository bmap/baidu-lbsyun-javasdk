...
public final class BaiduUtil {
	 ...
	 * @throws IOException
	 */
	public static String getSignature(String uri, Map<String, String> params,
			String secret, String method) throws IOException {
		Set<Entry<String, String>> entrys;
		if (method.equals("POST")) {
			// 先将参数以其参数名的字典序升序进行排序
			Map<String, String> sortedParams = new TreeMap<String, String>(
					params);
			entrys = sortedParams.entrySet();
		} else {
			entrys = params.entrySet();
		}
		// 遍历排序后的字典，将所有参数按"key=value"格式拼接在一起
		StringBuilder basesb = new StringBuilder();
		basesb.append(uri).append("?");
		for (Entry<String, String> param : entrys) {
			basesb.append(param.getKey()).append("=")
					.append(param.getValue()).append("&");
		}
		String basestring = basesb.substring(0, basesb.length()-1);
		basestring += secret;
		if (basestring.length() > 1000) {
			basestring = basestring.substring(0, 1000);
		}
