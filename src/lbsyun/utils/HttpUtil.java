...
public class HttpUtil {
	...
	private static HttpURLConnection getConnection(URL url, String method,
		...
		conn.setRequestProperty("User-Agent", "baidu-lbs-sdk-java-1.0");
	...
	public static String doPost(String url, Map<String, String> params,
		...
		conn.getOutputStream().write(
				buildQuery(params, charset).getBytes(charset));
		response = getResponseAsString(conn);
		return response;
	}
	 ...
	 *            请求的参数
	 * @param fileParams
	 *            文件上传的参数信息， key为文件的参数名称， value为FileItem对象
	 * @param connectTimeOut
	 *            url连接过期时间
	 * @param readTimeOut
	 *            请求读取过期时间
	...
	public static String uploadFile(String url, Map<String, String> params,
			Map<String, FileUtil> fileParams, int connectTimeOut,
			int readTimeOut) throws IOException {
		return uploadFile(url, params, fileParams, DEFAULT_CHARSET,
				connectTimeOut, readTimeOut);
	}

	public static String uploadFile(String url, Map<String, String> params,
			Map<String, FileUtil> fileParams, String charset,
			int connectTimeOut, int readTimeOut) throws IOException {
		HttpURLConnection conn = null;
		...
		if (params != null && !params.isEmpty()) {
			for (Entry<String, String> entry : params.entrySet()) {
				byte[] textBytes = getTextEntry(entry.getKey(),
		...
		if (fileParams != null && !fileParams.isEmpty()) {
			for (Entry<String, FileUtil> fileEntry : fileParams.entrySet()) {
				String fieldParam = fileEntry.getKey();
				FileUtil fileUtil = fileEntry.getValue();
				byte[] fileBytes = getFileEntry(fieldParam,
						fileUtil.getFileName(), fileUtil.getMimeType(), charset);
				out.write(entryBoundaryBytes);
				out.write(fileBytes);
				out.write(fileUtil.getContent());
			}
		}
	...
	public static Map<String, String> splitQuery(String query, String charset) {
		...
		if (!StringUtil.isEmpty(query)) {
			...
			for (String split : splits) {
				...
				if (StringUtil.areNotEmpty(keyAndValue)
						&& keyAndValue.length == 2) {
					try {
						ret.put(keyAndValue[0],
								URLDecoder.decode(keyAndValue[1], charset));
					} catch (UnsupportedEncodingException e) {
					}
				}
			}
